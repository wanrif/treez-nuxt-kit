...
<template>
  <div class="mx-auto max-w-4xl px-4 sm:px-6">
    <template v-if="mounted">
      <!-- Profile Header -->
      <div class="relative mb-6 overflow-hidden rounded-xl bg-white p-6 shadow sm:p-8 dark:bg-gray-800">
        <!-- Decorative Elements -->
          ...
          <div class="flex flex-col items-center space-y-6 sm:flex-row sm:items-start sm:justify-between sm:space-y-0">
            ...
            <div class="flex flex-col items-center space-y-4 sm:flex-row sm:items-start sm:space-y-0 sm:space-x-6">
              ...
              <div class="space-y-2 text-center sm:text-left">
                <h1
                  class="bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-2xl font-bold text-transparent sm:text-3xl dark:from-white dark:to-gray-300"
                >
                  {{ user?.name }}
                </h1>
                <div
      ...
      <!-- Profile Content -->
      <div class="rounded-xl bg-white p-4 shadow sm:p-6 dark:bg-gray-800">
        <form @submit.prevent="submit">
          <div class="space-y-6">
                  ...
                  >
                    <template v-if="key === 'bio'">
                      <UTextarea
                        ...
                        class="!w-full"
                        :ui="{
                          base: 'relative block w-full disabled:cursor-not-allowed disabled:opacity-50 border-0 shadow-sm bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white ring-1 ring-inset ring-gray-300 dark:ring-gray-700 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400',
                        }"
                      />
                    </template>
                    <template v-else>
            ...
            <div v-if="isEditing" class="flex flex-col space-y-3 sm:flex-row sm:justify-end sm:space-y-0 sm:space-x-4">
              <button
                type="button"
                class="order-2 w-full rounded-lg border border-gray-300 px-4 py-2 text-gray-700 transition duration-200 hover:bg-gray-50 sm:order-1 sm:w-auto dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700"
                @click="cancelEdit"
              >
                {{ $t('cancel') }}
              </button>
    ...
    <template v-else>
      ...
      <div class="mb-6 rounded-xl bg-white p-6 shadow-lg sm:p-8 dark:bg-gray-800">
        <div class="flex flex-col items-center space-y-6 sm:flex-row sm:items-start sm:justify-between sm:space-y-0">
          <div class="flex flex-col items-center space-y-4 sm:flex-row sm:space-y-0 sm:space-x-6">
            <div class="h-24 w-24 animate-pulse rounded-2xl bg-gray-200 sm:h-28 sm:w-28 dark:bg-gray-700" />
            <div class="space-y-2 text-center sm:text-left">
      ...
      <div class="rounded-xl bg-white p-4 shadow-sm sm:p-6 dark:bg-gray-800">
